...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def propagate_attention(self, g):
        ...
        # 소프트맥스 적용 전 어텐션 스코어 저장 (선택 사항)
        attn_scores_raw = g.edata['score'].clone()
        ...
        # 완전한 소프트맥스가 적용된 어텐션 스코어 계산
        # 각 엣지의 소프트맥스 값 = 해당 엣지의 exp(score) / 타겟 노드로 들어오는 모든 엣지의 exp(score) 합
        src, dst = g.edges()
        z = g.nodes[dst].data['z']
        
        # 각 엣지에 대해 정규화된 소프트맥스 값 계산
        # 정규화 상수로 나누어 완전한 소프트맥스 적용
        g.edata['softmax_score'] = g.edata['score'] / z[dst]
        
        # 완전한 소프트맥스가 적용된 어텐션 스코어 반환
        return g.edata['softmax_score'].clone()
    ...
    def forward(self, g, h, e):
        ...
        # 완전한 소프트맥스가 적용된 어텐션 스코어 얻기
        softmax_attn_scores = self.propagate_attention(g)
        
        h_out = g.ndata['wV'] / (g.ndata['z'] + torch.full_like(g.ndata['z'], 1e-6)) # adding eps to all values here
        e_out = g.edata['e_out']
        
        return h_out, e_out, softmax_attn_scores
...
class GraphTransformerLayer(nn.Module):
    ...
    def forward(self, g, h, e):
        ...
        h_attn_out, e_attn_out, attn_scores = self.attention(g, h, e)
